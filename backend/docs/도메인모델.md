...
- 하나의 방에서 이름은 중복될 수 없다.
- 메뉴는 필수이다.
- 모든 플레이어가 메뉴를 골라야 미니게임을 시작할 수 있다.
- 2~9명의 플레이어가 접속해 있어야 미니게임, 룰렛을 시작할 수 있다.
- PLAYING상태일 때만 룰렛을 돌릴 수 있다.
- READY, PLAYING상태일 때만 미니게임을 시작할 수 있다.
...
### 행위
- `joinPlayer()`: 플레이어가 참가한다.
- `addMiniGame()`: 미니게임을 추가한다.
- `hasNoMiniGames()`: 미니게임이 있는지 여부 반환
- `isInPlayingState()`: 플레이중인지 반환
- `hasEnoughPlayers()`: 게임하기에 인원이 충분한지 여부 반환
- `setPlaying()`: 플레이중으로 변경
