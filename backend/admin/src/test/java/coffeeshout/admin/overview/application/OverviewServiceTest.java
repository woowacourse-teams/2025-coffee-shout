...
class OverviewServiceTest {
    ...
    private OverviewService service() {
        return new OverviewService(overviewStatisticsRepository, reportAdminService,
                profanityAuditService, ipBlockAdminService, CLOCK);
    }
    ...
    class actionQueue {
        ...
        void 네_가지_대기_건수를_모은다() {
            ...
            given(ipBlockAdminService.getBlockedIps())
                    .willReturn(List.of(new BlockedIp("1.2.3.4", 60)));
        ...
        void 개수만_필요하므로_한_건만_조회한다() {
            ...
            then(profanityAuditService).should(times(2))
                    .listByStatus(any(), pageable.capture());
    ...
    class summaryOf {
        ...
        void 가입은_같은_경계를_Instant로_변환해_조회한다() {
            ...
            then(overviewStatisticsRepository).should()
                    .countSignupsBetween(from.capture(), to.capture());
        ...
        void 퍼널과_참여자와_가입_수를_함께_돌려준다() {
            ...
            given(overviewStatisticsRepository.countPlayersBetween(any(), any())).willReturn(42L);
            given(overviewStatisticsRepository.countSignupsBetween(any(), any())).willReturn(7L);
