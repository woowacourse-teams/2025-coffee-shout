...
public final class PlayerFixture {
    ...
    public static Player 게스트한스() {
        final Player 한스 = Player.createGuest(new PlayerName("한스"),  new SelectedMenu(MenuFixture.아메리카노(), MenuTemperature.ICE));
        한스.updateReadyState(true);
        return 한스;
    }

    public static Player 게스트꾹이() {
        final Player 꾹이 = Player.createGuest(new PlayerName("꾹이"),  new SelectedMenu(MenuFixture.아메리카노(), MenuTemperature.ICE));
        꾹이.updateReadyState(true);
        return 꾹이;
    }

    public static Player 게스트루키() {
        final Player 루키 = Player.createGuest(new PlayerName("루키"),  new SelectedMenu(MenuFixture.아메리카노(), MenuTemperature.ICE));
        루키.updateReadyState(true);
        return 루키;
    }

    public static Player 게스트엠제이() {
        final Player 엠제이 = Player.createGuest(new PlayerName("엠제이"),  new SelectedMenu(MenuFixture.아메리카노(), MenuTemperature.ICE));
