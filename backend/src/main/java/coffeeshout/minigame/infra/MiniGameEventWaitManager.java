...
@Slf4j
@Component
public class MiniGameEventWaitManager {
    ...
    public <T> CompletableFuture<T> registerWait(String eventId) {
        ...
        log.debug("미니게임 이벤트 Future 등록: eventId={}", eventId);

        // 자동 정리: Future 완료 시 pendingEvents에서 제거
        future.whenComplete((result, throwable) -> {
            pendingEvents.remove(eventId);
            if (throwable != null) {
                log.error("미니게임 이벤트 자동 정리 (실패): eventId={}", eventId, throwable);
                return;
            }
            log.debug("미니게임 이벤트 자동 정리 (성공): eventId={}", eventId);
        });
    ...
    public <T> void notifySuccess(String eventId, T result) {
        ...
        if (future == null) {
            log.warn("미니게임 이벤트 성공 알림 실패: eventId={}에 대한 Future가 존재하지 않습니다.", eventId);
            return;
        }

        future.complete(result);
        log.debug("미니게임 이벤트 성공 알림: eventId={}", eventId);
    }
    ...
    public void notifyFailure(String eventId, Throwable throwable) {
        ...
        if (future == null) {
            log.warn("미니게임 이벤트 실패 알림 시도: eventId={}에 해당하는 Future가 존재하지 않습니다.", eventId);
            return;
        }

        future.completeExceptionally(throwable);
        log.debug("미니게임 이벤트 실패 알림: eventId={}", eventId);
    }
    ...
    public void forceCleanup(String eventId) {
        ...
        if (future != null) {
            future.cancel(true);
            log.warn("미니게임 이벤트 강제 정리: eventId={}", eventId);
        }
    }
}
