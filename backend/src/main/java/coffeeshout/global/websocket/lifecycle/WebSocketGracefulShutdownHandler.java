...
public class WebSocketGracefulShutdownHandler implements SmartLifecycle {
    ...
    public void stop(@NonNull Runnable callback) {
        ...
        final long timeoutMinutes = shutdownWaitDuration.toMinutes();
        log.info("⏳ {} 개의 활성 WebSocket 연결 종료 대기 중... (최대 {}분)", currentConnections, timeoutMinutes);
        ...
        try {
            shutdownFuture.get(timeoutMinutes, TimeUnit.MINUTES);
            log.info("✅ 모든 WebSocket 연결 정상 종료 완료");
        } catch (TimeoutException e) {
            final int remaining = sessionTracker.getActiveSessionCount();
            log.warn("⚠️ Graceful Shutdown 타임아웃 ({}분): 활성 연결 {} 개가 남아있습니다. 강제 종료합니다.",
                    timeoutMinutes, remaining);
