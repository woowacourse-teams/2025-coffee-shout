...
public class QrCodeService {
    ...
    public void generateQrCodeAsync(String joinCode) {
        ...
        try {
            ...
            // 3. Room에 저장
            publishManager.publishRoom(new QrCodeStatusEvent(joinCode, QrCodeStatus.SUCCESS, qrCodeUrl));
            log.info("QR 코드 생성 완료: joinCode={}, url={}", joinCode, qrCodeUrl);
        } catch (Exception e) {
            log.error("QR 코드 생성 실패: joinCode={}, error={}", joinCode, e.getMessage(), e);

            publishManager.publishRoom(new QrCodeStatusEvent(joinCode, QrCodeStatus.ERROR, null));
        }
    }
