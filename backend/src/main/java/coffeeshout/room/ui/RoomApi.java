...
public interface RoomApi {
    ...
    ResponseEntity<RoomCreateResponse> createRoom(@Valid @RequestBody RoomEnterRequest request);

    @Operation(summary = "방 참가", description = "joinCode를 사용하여 방에 참가합니다.")
    CompletableFuture<ResponseEntity<RoomEnterResponse>> enterRoom(
            @Parameter(description = "방 입장 코드", required = true) @PathVariable String joinCode,
    ...
    ResponseEntity<JoinCodeExistResponse> checkJoinCode(
            @Parameter(description = "방 입장 코드", required = true) @RequestParam String joinCode
    );

    @Operation(summary = "게스트 이름 중복 확인", description = "방 내에서 게스트 이름이 중복되는지 확인합니다.")
    ResponseEntity<GuestNameExistResponse> checkGuestName(
            @Parameter(description = "방 입장 코드", required = true) @RequestParam String joinCode,
            @Parameter(description = "게스트 이름", required = true) @RequestParam String guestName
    );
    ...
    ResponseEntity<Void> kickPlayer(
            @Parameter(description = "방 입장 코드", required = true) @PathVariable String joinCode,
            @Parameter(description = "플레이어 이름", required = true) @PathVariable String playerName
    );
}